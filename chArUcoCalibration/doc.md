## chArUcoCalibrationを実装してみた

---
### ディレクトリ内の画像群読み込む関数
1. dir_pathに画像群を置いているディレクトリを指定する
2. dir_pathのディレクトリ内にあるファイルをcv::Matのvectorに格納する
3. 読み込んだファイル群を返す
```c++
...
```
---
### カメラパラメータをyaml形式で保存する関数
1. file_dirに出力するカメラパラメータのファイルパスを設定
2. cameraMatrix, distCoeffs, reproErrに保存するパラメータ、再投影誤差を設定
```c++
